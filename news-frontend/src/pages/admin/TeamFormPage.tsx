...
export default function TeamFormPage() {
  ...
  useEffect(() => { if (id) apiService.getTeamMemberById(parseInt(id)).then(m => setForm({ name: m.name, position: m.position, description: m.description, email: m.email || '', phone: m.phone || '', order: m.order, active: m.active })); }, [id]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try { if (isEdit) await apiService.updateTeamMember(parseInt(id!), fd); else await apiService.createTeamMember(fd); navigate('/admin/team'); } catch (e) { console.error(e); } finally { setSaving(false); }
  };
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit} className="card p-6 space-y-5 max-w-2xl">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div><label className="block text-sm font-medium mb-1.5">Порядок</label><input type="number" value={form.order} onChange={e => setForm(f=>({...f,order:parseInt(e.target.value)}))} className="w-full px-4 py-2.5 border rounded-lg text-sm focus:ring-2 focus:ring-primary-500"/></div>
          <div><label className="block text-sm font-medium mb-1.5">Фото</label><label className="flex items-center gap-2 px-4 py-2.5 border rounded-lg text-sm cursor-pointer hover:bg-gray-50"><Upload size={16}/>{photo ? photo.name : 'Обрати файл'}<input type="file" accept="image/*" onChange={e => setPhoto(e.target.files?.[0] || null)} className="hidden"/></label></div>
        </div>
